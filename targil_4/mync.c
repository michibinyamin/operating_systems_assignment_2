    ...
    for (int i = isE; i < argc; i += 2) {
        ...
        if (strcmp(argv[i], "-i") == 0) {
            ...
            }else if (result2 != NULL) // If TCPC
            {
                ...
                got_input = 2;  // Change to TCPC
            }else if (result3 != 0)  // If UDPS
            {
                got_input = 3;  // Change to UDPS
            }
    ...
    if (isE == 3)   // id -e run the program
    {
        if (got_output == 3)    
        {
            ...
            if (pid == 0)
            {
                // Child process
                close(fd[1]);
                struct sockaddr_in server_addr = get_server(output_ip,C_output_port);
                char buffer[1024];
                while (1)
                {
                    ...
                    if (len > 1) { // Data entered
                        // Send data to server
                        int bytes_sent = sendto(sockfd_output, buffer, bytes_read, 0,(struct sockaddr *)&server_addr, sizeof(server_addr));
                        if (bytes_sent < 0) {
